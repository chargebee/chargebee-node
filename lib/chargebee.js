...
ChargeBee._core = (function() {
        ...
        return function(res) {
            ...
            res.on('end', function() {
                try {
                    ...
                    throwError(callBack, 500, 'invalid_json', 'invalid json from chargebee Api');
                }
            });
        };
    };
    ...
    coreRef.errorHandler = function(req, callBack) {
        return function(error) {
            if (req._isAborted)
                return;
            throwError(callBack, 503, 'connection_error', 'connection error while making request.');
        };
    };
                ...
                for (var arrIdx = 0; arrIdx < value.length; arrIdx++) {
                    if (typeof value[arrIdx] === 'object' || ChargeBee._util.isArray(value[arrIdx])) {
                        ...
                        if (typeof value[arrIdx] !== 'undefined' && ChargeBee._util.trim(value[arrIdx]) !== '') {
                            serialized.push(encodeURIComponent(key) + "=" + (encodeURIComponent(value[arrIdx])));
                        }
                    }
                }
    ...
    var throwError = function(callBack, httpStatusCode, errorCode, message) {
        var error = {
